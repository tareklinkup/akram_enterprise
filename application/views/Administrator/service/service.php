...
<script>
	...
	new Vue({
		...
		methods: {
			...
			addToCart() {
				if (this.selectedProduct.Product_Name == '' || this.product.model == '' || this.product.imei == '' || this.product.quantity == 0) {
				...
				if (this.cartProductEditIndex != null) {
					...
					let checkProduct = this.cart.findIndex(item => item.Product_Name == this.selectedProduct.Product_Name && item.imei == this.product.imei);
					if (checkProduct > -1) {
			...
			async saveService() {
				...
				this.service.total = this.costCart.reduce((p, c) => {
					return +p + +c.amount
				}, 0);
			...
			editCartItem(index) {
				...
				this.selectedProduct = this.products.find(item => item.Product_Name == cartProduct.product_name);
				this.cartProductEditIndex = index;
			},
			getServices() {
				axios.post('/get_service', {
						invoice: this.service.invoice
					})
					.then(res => {
						let r = res.data;
