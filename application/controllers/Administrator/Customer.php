    ...
    public function addCustomerPayment()
    {
        ...
        try {
            ...
            if ($paymentObj->CPayment_TransactionType == 'CR') {
                $currentDue = $paymentObj->CPayment_TransactionType == 'CR' ? $paymentObj->CPayment_previous_due - ($paymentObj->CPayment_amount + $paymentObj->CPayment_discount_amount) : $paymentObj->CPayment_previous_due + ($paymentObj->CPayment_amount + $paymentObj->CPayment_discount_amount);
        ...
        $payments = $this->db->query("
            ...
            select
                ...
                concat('Received - ', 
                    ...
                    end, ' ', cp.CPayment_notes, concat(' Discount - ', cp.CPayment_discount_amount)
                ) as description,
                0.00 as bill,
                (cp.CPayment_amount + cp.CPayment_discount_amount) as paid,
                0.00 as due,
                0.00 as returned,
                0.00 as paid_out,
                0.00 as balance
            ...
            select
                ...
                0.00 as due,
                0.00 as returned,
                (cp.CPayment_amount + cp.CPayment_discount_amount) as paid_out,
                0.00 as balance
            from tbl_customer_payment cp
            left join tbl_bank_accounts ba on ba.account_id = cp.account_id
