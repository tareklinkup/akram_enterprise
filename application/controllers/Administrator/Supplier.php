...
class Supplier extends CI_Controller {
        ...
        $payments = $this->db->query("
            ...
            select
                ...
                concat('Paid - ', 
                    ...
                    end, ' ', sp.SPayment_notes, concat(' Discount - ', sp.SPayment_discount_amount)
                ) as description,
                0.00 as bill,
                (sp.SPayment_amount + sp.SPayment_discount_amount) as paid,
                0.00 as due,
                0.00 as returned,
                0.00 as cash_received,
                0.00 as balance
            ...
            select 
                ...
                concat('Received - ', 
                    ...
                    end, ' ', sp2.SPayment_notes, concat(' Discount - ', sp2.SPayment_discount_amount)
                ) as description,
                0.00 as bill,
                0.00 as paid,
                0.00 as due,
                0.00 as returned,
                (sp2.SPayment_amount + sp2.SPayment_discount_amount) as cash_received,
                0.00 as balance
    ...
    public function addSupplierPayment(){
        ...
        try{
            ...
            if ($paymentObj->SPayment_TransactionType == 'CP') {
                $currentDue = $paymentObj->SPayment_TransactionType == 'CP' ? $paymentObj->SPayment_previous_due - ($paymentObj->SPayment_amount + $paymentObj->SPayment_discount_amount) : $paymentObj->SPayment_previous_due + ($paymentObj->SPayment_amount + $paymentObj->SPayment_discount_amount);
                
                if($paymentObj->sendSMS){
                    //Send sms
                    $supplierInfo = $this->db->query("select * from tbl_supplier where Supplier_SlNo = ?", $paymentObj->SPayment_customerID)->row();
                    $sendToName = $supplierInfo->Supplier_Name;
                    
                    $message = "মি: {$sendToName},\nবিল পেমেন্ট: Tk.{$paymentObj->SPayment_amount}.\nমোট বাকী: Tk.{$currentDue}";
