...
class Model_Table extends CI_Model{
    ...
    public function supplierDue($clauses = "" , $date = null) {
        ...
        $supplierDues = $this->db->query("
            ...
            (select ifnull(sum(sp.SPayment_discount_amount), 0.00) from tbl_supplier_payment sp 
                where sp.SPayment_customerID = s.Supplier_SlNo 
                and sp.SPayment_TransactionType = 'CP'
                " . ($date == null ? "" : " and sp.SPayment_date < '$date'") . "
                and sp.SPayment_status = 'a'
            ...
            (select ifnull(sum(sp2.SPayment_discount_amount), 0.00) from tbl_supplier_payment sp2 
                where sp2.SPayment_customerID = s.Supplier_SlNo 
                and sp2.SPayment_TransactionType = 'CR'
                " . ($date == null ? "" : " and sp2.SPayment_date < '$date'") . "
                and sp2.SPayment_status = 'a'
            ) as cashReceivedDiscount,
            ...
            (select invoicePaid + cashPaid + cashPaidDiscount) as paid,
            
            (select (bill + cashReceived + cashReceivedDiscount) - (paid + returned)) as due
    ...
    public function customerDue($clauses = "" , $date = null) {
        ...
        $dueResult = $this->db->query("
            ...
            (select ifnull(sum(cp.CPayment_discount_amount), 0.00) 
                from tbl_customer_payment cp 
                where cp.CPayment_customerID = c.Customer_SlNo 
                and cp.CPayment_TransactionType = 'CR'
                " . ($date == null ? "" : " and cp.CPayment_date < '$date'") . "
                and cp.CPayment_status = 'a') as cashReceivedDiscount,
            ...
            (select ifnull(sum(cp.CPayment_discount_amount), 0.00) 
                ...
                and cp.CPayment_TransactionType = 'CP'
                " . ($date == null ? "" : " and cp.CPayment_date < '$date'") . "
                and cp.CPayment_status = 'a') as paidOutAmountDiscount,
            ...
            (select invoicePaid + cashReceived + cashReceivedDiscount) as paidAmount,

            (select (billAmount + paidOutAmount + posReturnAmount + paidOutAmountDiscount) - (paidAmount + returnedAmount)) as dueAmount
    ...
    public function specialCustomerDue($clauses = "") {
        ...
        $dueResult = $this->db->query("
            ...
            (select (c.previous_due + saleInvoiceDue + supplierPaid + purReturned + customerPaid + customerPaidDiscount) - (purchaseInvoiceDue + cashReceived + cashReceivedDiscount + returnedAmount + supplierReceived)) as dueAmount
